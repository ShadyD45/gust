...
export class RunRecorder {
    ...
    constructor({ agentName, agentVersion, taskInput, taskId, runId, gitCommit, taskContext, }) {
        ...
        if (gitCommit)
            this._agent.git_commit = gitCommit;
        this._task = { id: taskId || this.runId, input: taskInput };
        if (taskContext)
            this._task.context = taskContext;
        this._spans = [];
        this._outcome = null;
        this._metadata = {};
        if (envSample)
    ...
    span(name, { spanType = "agent", attributes, parentSpanId, } = {}) {
        ...
        if (!name)
            throw new AgentRunError("span name is required");
        const span = {
    ...
    complete(output = "", durationNs) {
        ...
        if (durationNs !== undefined)
            this._outcome.duration_ns = durationNs;
    }
...
export function resolveIngestUrl(url) {
    ...
    if (!dest) {
        ...
        if (otel)
            dest = otel.replace(/\/$/, "") + "/v1/runs";
    }
    if (!dest) {
        throw new AgentRunError("no ingest URL; pass url or set AGENTEVAL_INGEST_URL / OTEL_EXPORTER_OTLP_ENDPOINT");
    }
    dest = dest.replace(/\/$/, "");
    if (!dest.endsWith("/v1/runs"))
        dest = dest + "/v1/runs";
    return dest;
}
