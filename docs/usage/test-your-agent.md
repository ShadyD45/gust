...
`gust test` always starts the fixture proxy from resolved fixtures and `--fixtures`. Ordered-sequence fixtures force `--concurrency 1` so counters do not bleed across workers.
