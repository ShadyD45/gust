...
Looking for task-oriented guides rather than design rationale? See [Usage]({% link usage/index.md %}) to run gust against your own agent, [Tuning the gate]({% link usage/tuning.md %}) for policy and execution knobs, the [live-agent demo]({% link usage/live-agent-demo.md %}) for a recorded Mode 3 run, and [Extending]({% link extending/index.md %}) to add evaluators, runners, or cross-language plugins.
